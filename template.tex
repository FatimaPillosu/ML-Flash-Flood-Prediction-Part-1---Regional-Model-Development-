...
Flash floods represent one of the most devastating natural disasters worldwide, causing significant economic damage and loss of life. With climate change expected to increase the intensity and frequency of flash-flood-triggering rainfall events, there is an urgent need for improved early warning systems capable of providing medium-range forecasts for better preparedness and emergency response. This study develops and evaluates data-driven models for predicting areas at risk of flash floods over the Continental United States using hydro-meteorological reanalysis data and flash flood impact reports. Six machine learning architectures were systematically compared: three different gradient boosting implementations (XGBoost, LightGBM, CatBoost), two random forest implementations (XGBoost-RF, LightGBM-RF), and a feed-forward neural network. Models were trained using a repeated nested stratified cross-validation framework on the severely imbalanced observation dataset, spanning 2001-2020. The models were independently verified on 2021-2024 observational data. Input features included rainfall probability estimates, antecedent soil moisture, vegetation indices, and topographic characteristics. Loss functions specific to imbalanced datasets were tested in each implementation alongside more standard functions. Results considering reanalysis data demonstrate that gradient boosting implementations, particularly XGBoost, achieve the best performance in terms of reliability, discrimination ability, and operational concerns (e.g., time for training). Neural networks showed similar performance compared to gradient boosting implementations, whilst requiring approximately 40 times longer training duration. Shapley Additive exPlanations (SHAP) analysis reveals that rainfall dominates model decisions, whilst hydrological and topographic features provide critical modulating effects. Models trained with balanced loss functions demonstrate superior reliability compared to weighted approaches, maintaining acceptable false alarm rates essential for operational deployment. Forecast skill assessment shows minimal degradation in discrimination ability up to day 5 predictions, though precision-recall metrics exhibit a steeper decline. The models maintain reliable probability estimates for forecast probabilities up to 10\% for reanalysis data and below 2\% for day 5 forecasts. A case study of Storm Ida demonstrates the models' capability to identify flash flood risk areas with high correspondence to observed impacts. This research establishes the feasibility of regional-scale, data-driven flash flood prediction and provides clear operational protocols: high-confidence warnings for 0-24 hour forecasts may support protective actions, whilst longer lead time predictions enable preparedness measures. Future developments should prioritise integration with higher-resolution numerical weather prediction models and ensemble approaches for robust uncertainty quantification in operational forecasting systems.
